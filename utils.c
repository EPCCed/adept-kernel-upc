...
void upc_loop_timer_nop(unsigned long limit){

  struct timespec t1, t2;
  int index;

  clock_gettime(CLOCK, &t1);
  upc_forall(index=0; index<limit*THREADS; index++; index) {
    __asm__ ("nop");
  }
  clock_gettime(CLOCK, &t2);

  if (MYTHREAD == 0) elapsed_time_hr(t1, t2, "UPC forall Loop on its own with nop");
}
...
void upc_barrier_timer(){
  ...
  if (MYTHREAD == 0) elapsed_time_hr(t1, t2, "UPC barrier");
}
